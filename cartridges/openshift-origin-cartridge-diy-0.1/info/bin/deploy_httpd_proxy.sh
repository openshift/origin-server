...
cat <<EOF > "/etc/httpd/conf.d/openshift origin/${uuid}_${namespace}_${application}/${cartridge_type}.conf"
  Alias /health $CART_INFO_DIR/configuration/health.html
EOF

cat <<EOF > "/etc/httpd/conf.d/openshift origin/${uuid}_${namespace}_${application}/zzzzz_proxy.conf"
  ProxyPass /health !
  ProxyPass / http://$IP:8080/ status=I
  ProxyPassReverse / http://$IP:8080/
EOF

cat <<EOF > "/etc/httpd/conf.d/openshift origin/${uuid}_${namespace}_${application}/00000_default.conf"
...
cat <<EOF > "/etc/httpd/conf.d/openshift origin/${uuid}_${namespace}_${application}.conf"
<VirtualHost *:80>
  RequestHeader append X-Forwarded-Proto "http"

  Include /etc/httpd/conf.d/openshift origin/${uuid}_${namespace}_${application}/*.conf
</VirtualHost>
