class UserController < BaseController
  ...
  def destroy
    ...
    return render_format_error(:forbidden, "User deletion not permitted. Only applicable for subaccount users.",
                               138, "DELETE_USER") unless @cloud_user.parent_user_login
    if !force && (!@cloud_user.domains.empty? || !@cloud_user.applications)
      return render_format_error(:unprocessable_entity, "User '#{@login}' has valid domains or applications. Either delete domains/applications and retry the operation or use 'force' option.", 139, "DELETE_USER")
    end
