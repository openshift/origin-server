class EmbCartController < BaseController
  ...
  def update
    ...
    storage_map = {}
    app.comp_instance_map.values do |cinst|
    ...
    return render_error(:not_found, "Cartridge '#{cartridge_name}' for application '#{app_id}' not found",
                        163, "UPDATE_CARTRIDGE") unless cart_comp_inst
    ...
    if additional_storage
      ...
      num_storage = nil
      begin 
        num_storage = Integer(addtional_storage)
      rescue => e
      ...
      return render_format_error(:unprocessable_entity, "Storage value must be between #{min_storage} and #{max_storage}", 166,
                                   "UPDATE_CARTRIDGE") if (num_storage < min_storage) or (num_storage > max_storage)
      begin
        storage_map.each do |group_name, component_instance_list|
          each_component_share = Float(num_storage)/component_instance_list
          ginst = app.group_instance_map[group_name]
          component_instance_list.each { |cinst| cinst.set_additional_quota(each_component_share) }
        end
        app.save
      rescue Exception => e
