...
Feature: Application template list
  ...
  Scenario: Add a template and create an application
    ...
    When I create a new application named 'fooapp' with the template
    Then the response should be "201"
  ...
  Scenario: Test deploying a rails application and check if gem dependencies are resolved
    ...
    Then the response should be "201"
    When I create a new application named 'fooapp' within domain 'cucumber<random>' with the template
    Then the response should be "201"
    And the application should be accessable
    When I send a DELETE request to "/domains/cucumber<random>/applications/fooapp"
    Then the response should be "204"
