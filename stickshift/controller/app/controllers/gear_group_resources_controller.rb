class GearGroupResourcesController < BaseController
  ...
  def update
    ...
    return render_format_error(:not_found, "User is not allowed to change storage quota", 164,
                               "UPDATE_GEAR_GROUP_RESOURCES") unless max_storage
    
    begin 
      num_storage = Integer(storage)
    rescue => e
      return render_format_error(:not_found, "Invalid storage value provided.", 165, "UPDATE_GEAR_GROUP_RESOURCES", "storage")
    end
    ...
    return render_format_error(:not_found, "Gear group '#{gear_group_id}' for application '#{app_id}' not found",
    ...
    begin
      ...
      return render_format_error(:not_found, "Storage value must be between #{min_storage} and #{max_storage}", 166,
      ...
      return render_format_exception(e, "UPDATE_GEAR_GROUP_RESOURCES")
    end
    
    resources = RestGearGroupResources.new(selected_gear_group.uuid, selected_gear_group.get_quota()[:storage])
    render_format_success(:ok, "resources", resources, "UPDATE_GEAR_GROUP_RESOURCES",
