...
class Application
  ...
  def add_features(features, group_overrides=[], init_git_url=nil)
    ssl_endpoint = Rails.application.config.openshift[:ssl_endpoint] or "allow"
    ...
    features.each do |feature_name|
      ...
      if cart.is_web_framework? and defined?(cart.endpoints) and cart.endpoints.respond_to?(:each)
        ...
        if (((ssl_endpoint == "deny") and cart_req_ssl_endpoint ) or
          ...
          raise OpenShift::UserException.new("Invalid cartridge '#{feature}' conflicts with platform SSL_ENDPOINT setting.", 109, "cartridge")
        end
      end
