  ...
  def elaborate(app)
    ...
    start_order.each do |component_instance|
      component_instance.group_instance.get_gears(component_instance).each do |gear|
        ops.push(TidyCompOp.new(group_instance_id: component_instance.group_instance._id.to_s, gear_id: gear._id.to_s, comp_spec: {'cart' => component_instance.cartridge_name, 'comp' => component_instance.component_name}))
      end
    end
