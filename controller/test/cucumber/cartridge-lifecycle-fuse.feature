...
Feature: Cartridge Lifecycle Fuse Verification Tests
  Scenario Outline: Application Creation
    ...
    When a container named cbr is created using the example-quickstarts-cbr profile
    Then 2 containers should exist
    
    Given an existing application named cbr
    Then 1 camel route should exist on app cbr
    When input files are copied from \$OPENSHIFT_FUSE_DIR/container/fabric/import/fabric/configs/versions/1.0/profiles/example-quickstarts-cbr/data/* to  \$OPENSHIFT_FUSE_DIR/container/work/cbr/input
