...
Feature: Rest Quick tests
  ...
  Scenario Outline: Typical Workflow
    ...
    Then the response should be "200"
    When I send a POST request to "/domains/api<random>/applications/app/aliases" with the following:"id=alias<random>"
    Then the response should be "201"
    When I send a DELETE request to "/domains/api<random>/applications/app/aliases/alias<random>" 
