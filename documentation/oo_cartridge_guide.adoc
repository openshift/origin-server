...
Note that when the force_npm_deploy marker is present, the hot_deploy marker
is not supported. In this case when you do add hot_deploy marker, the start
operation will fail.

[NOTE]
====
The `hot_deploy` marker is restricted to the supervisor only. Using the
`force_npm_deploy` marker, will result in your application not being started.
====
