...
=== Template Repository Layout
----
wsgi.py                WSGI entry-point <1>
setup.py               Standard setup.py, specify deps here <2>
.openshift/            Location for OpenShift specific files
    action_hooks/      See the Action Hooks documentation <3>
    markers/           See the Markers section below
----
<1> For backward compatibility, `wsgi/application` entry-point is also supported with higher priority.
    You can customize the path to the WSGI entry point, see the Environment variables section below.
<2> Adding deps to the `install_requires` will cause the cartirdge to install those deps at git push time.
<3> link:oo_user_guide.html#action-hooks[Action Hooks] documentation
...
OPENSHIFT_PYTHON_WSGI_APPLICATION:: Set an alternative path to the WSGI entry-point
