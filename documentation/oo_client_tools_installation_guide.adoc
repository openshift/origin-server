...
* If you want to use https://rvm.io/[Ruby Version Manager (rvm)], see "<<rvm_and_rhc,Ruby Version Manager and `rhc`>>".
* If your OpenShift deployment uses http://en.wikipedia.org/wiki/Kerberos_%28protocol%29[Kerberos] for authentication, see "<<kerberos_and_rhc,Kerberos and `rhc`>>".
...
[[kerberos_and_rhc]]
== Kerberos and `rhc`
If the OpenShift deployment that you are working with uses http://en.wikipedia.org/wiki/Kerberos_%28protocol%29[Kerberos] as the authentication mechanism, you will need to install an additional Ruby gem wherever you are planning on running `rhc`:

----
$ gem install gssapi
----

This will enable `rhc` to interact with Kerberos.
