...
module OpenShift
  ...
  class Node < Model
    ...
    def self.init_pam_limits(uuid, limits={})
      ...
      limits_file = File.join(DEFAULT_PAM_LIMITS_DIR, "#{limits_order}-#{uuid}.conf")
    ...
    def self.get_pam_limits(uuid)
      ...
      begin
        File.open(limits_file, O_RDONLY) do |f|
          f.each do |l|
            l.gsub!(/\#.*$/,'')
            l.strip!
            l.chomp!
    ...
    def self.pam_freeze(uuid)
      ...
      limits["nproc"]=0
      self.set_pam_limits(uuid, limits)
    end
