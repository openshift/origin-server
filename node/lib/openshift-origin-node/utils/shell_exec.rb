  ...
  module OpenShift::Runtime::Utils
    module ShellExec
      def shellCmd(cmd, pwd = ".", ignore_err = true, expected_rc = 0, timeout = 3600)
        OpenShift::Runtime::Utils::ShellExec.shellCmd(cmd, pwd, ignore_err, expected_rc, timeout)
      end
      ...
      #
      # Examples
      #   OpenShift::Runtime::Utils::ShellExec.shellCmd('ls /etc/passwd')
      #   # => ["/etc/passwd\n","", 0]
      #
      ...
      def self.shellCmd(cmd, pwd = ".", ignore_err = true, expected_rc = 0, timeout = 3600)
        ...
        begin
          ...
          raise OpenShift::Runtime::Utils::ShellExecutionException.new(e.message, rc, out, err
                ) unless ignore_err
        end
      ...
      def self.run_as(uid, gid, cmd, pwd = ".", ignore_err = true, expected_rc = 0, timeout = 3600)
        mcs_level, err, rc = OpenShift::Runtime::Utils::ShellExec.shellCmd("/usr/bin/oo-get-mcs-level #{uid}", pwd, true, 0, timeout)
        raise OpenShift::Runtime::Utils::ShellExecutionException.new(
        ...
        pid     = fork {
          ...
          out, err, rc = OpenShift::Runtime::Utils::ShellExec.shellCmd(command, pwd, true, 0, timeout)
          exit $?.exitstatus
        }
