...
class OpenshiftTest < MiniTest::Unit::TestCase
  ...
  def test_cartridge_do_action
    ...
    request.expects(:[]=).with(:args, {}).returns {}
    @agent.expects(:request).returns(request).twice
  ...
  def test_execute_parallel_action
    ...
    reply_contents = [{:job => {:action => 'test-openshift', :args => {:arg1 => true}}, :result_exit_code => 0, :result_stdout => 'test output', :result_addtl_params => nil}]
    reply          = mock('reply')
