...
class ApplicationContainerTest < Test::Unit::TestCase
  ...
  def setup
    ...
    OpenShift::V1CartridgeModel.any_instance.stubs(:tidy).returns(nil)
    OpenShift::V2CartridgeModel.any_instance.stubs(:tidy).returns(nil)
  end

  def test_public_endpoint_create
    ...
    @container.user.expects(:add_env_var).returns(nil).times(3)
  ...
  def test_endpoint_delete
    ...
    delete_all_args = [@ports_begin, @ports_begin+1, @ports_begin+2]
    proxy.expects(:delete_all).with(delete_all_args, true).returns(nil)

    @container.user.expects(:remove_env_var).returns(nil).times(3)
